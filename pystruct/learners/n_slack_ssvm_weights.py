...
class NSlackSSVMWeighted(BaseSSVM):
    ...
    This version implements sample weights.
    ...
    * Joachims, Thorsten and Finley, Thomas and Yu, Chun-Nam John:
    ...
    def _solve_n_slack_qp(self, constraints, n_samples, weights):
        C = np.float(self.C)
        ...
        # box constraint: sum of all alpha for one example must be <= C/n * vi
    ...
    def fit(self, X, Y, weights,constraints=None, warm_start=None, initialize=True):
        ...
        if constraints is None:
            ...
            # warm start
            objective = self._solve_n_slack_qp(constraints, n_samples, weights)
        try:
